...
class VaccinationManager {
    friend int main();
    private:
        // Using vector to store vaccinations
        vector<Vaccination> doses;
...
int VaccinationManager::findIndexById(const string& VaccinationId) {
    // Using the .find method of vector to find the element
    auto it = find_if(doses.begin(), doses.end(), 
        [&VaccinationId](const Vaccination& v) {
            return v.VaccinationId == VaccinationId;
        });

    if (it != doses.end()) {
        return it - doses.begin(); // Return the index by calculating the difference between the iterator and the begin
    }
...
void VaccinationManager::deleteById() {
    ...
    // Erase the vaccination from the vector using .erase
...
// int VaccinationManager::findIndexById(const string& VaccinationId) {
//     // Simple alternative using std::find
...
//     // Erase the vaccination from the vector
//     doses.erase(doses.begin() + index);
